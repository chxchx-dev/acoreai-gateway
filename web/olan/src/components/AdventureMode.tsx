...
import alaniaIndicating from '../assets/alania/alania_indicating.png';
import alaniaHeart from '../assets/alania/alania_heart.png';
import alaniaNopasa from '../assets/alania/alania_nopasa.png';
import alaniaIdea from '../assets/alania/alania_idea.png';
import alaniaStuding from '../assets/alania/alania_studing.png';
    ...
    return (
      <div className="av2-root">
        ...
        <div className="av2-init-screen">
          ...
          <p className="av2-init-sub">AlanIA preparará tu camino de aprendizaje personalizado 🌟</p>
...
// ── Alania character helper ───────────────────────────────────────────────────

type AlaniaGesture = 'hola' | 'indicating' | 'idea' | 'studing' | 'heart' | 'nopasa';

const ALANIA_IMG: Record<AlaniaGesture, string> = {
  hola: alaniaHola,
  indicating: alaniaIndicating,
  idea: alaniaIdea,
  studing: alaniaStuding,
  heart: alaniaHeart,
  nopasa: alaniaNopasa,
};

const ALANIA_MESSAGES: Record<AlaniaGesture, string[]> = {
...
function getRandomMsg(gesture: AlaniaGesture): string {
  const msgs = ALANIA_MESSAGES[gesture];
  return msgs[Math.floor(Math.random() * msgs.length)];
}

function AlaniaPanel({ gesture }: { gesture: AlaniaGesture }) {
  const [msg] = useState(() => getRandomMsg(gesture));
  return (
    <div className={`al-panel al-panel--${gesture}`}>
      <img src={ALANIA_IMG[gesture]} alt="Alania" className="al-img" />
...
function LessonPlayer({
  ...
  // Alania gesture based on current state
  const gesture: AlaniaGesture =
    step === 'done' ? 'hola'
    : step === 'vocab' ? 'studing'
  ...
  return (
    <div className="av2-player">
      ...
      {/* Main layout: Alania + Content */}
      <div className="av2-lesson-layout">
        {/* Alania character (left / top) */}
        <AlaniaPanel key={`${gesture}-${practiceIdx}`} gesture={gesture} />

...
import { ALANIA_MASTER_PROMPT, ENGLISH_LEVELS, type EnglishLevelKey, type EnglishLevelDef, type TopicCategory } from "./lib/prompt";
import alania from "./assets/alania/alania.png";
...
import logoAiAlania from "./assets/alania/logo-ai-alania.png";
  ...
  return (
    <div className="persp-container">
      ...
      <div className="persp-header">
        <img src={alaniaIdea} alt="" className="persp-avatar" />
        <div className="persp-header-text">
          <strong>AlanIA · Análisis Crítico</strong>
      ...
      {!msg.streaming && (
        <div className="persp-compare">
          <img src={alaniaIdea} alt="" className="persp-compare-avatar" />
...
const PROYECTO_KEY = 'olan-web:proyecto';
...
function parseRedirectParams(): RedirectParams | null {
  const path   = window.location.pathname;
  const search = window.location.search;
  const qs     = new URLSearchParams(search);

  // Formato soportado:
  // /validar/{identificacion}/{token}/{proyecto}/
  const plain = path.match(/^\/validar\/([^/]+)\/([^/]+)(?:\/([^/]+))?\/?$/);
  if (!plain) return null;

  const proyecto = (plain[3] ? decodeURIComponent(plain[3]) : undefined)
    || qs.get('proyecto') || undefined;

  if (proyecto) window.localStorage.setItem(PROYECTO_KEY, proyecto);

  return {
    identificacion: decodeURIComponent(plain[1]),
    olanToken:      decodeURIComponent(plain[2]),
    proyecto,
  };
}

function getProyecto(): string | null {
  return window.localStorage.getItem(PROYECTO_KEY);
}

function buildRedirectUser(params: RedirectParams): DemoUser {
  const id = params.identificacion.trim();

  return {
    id,
    email: `${id}@olan.internal`,
    name: id,
    role: 'ACADEMIC',
    token: '',
  };
...
export function App() {
  const [redirectParams] = useState(() => parseRedirectParams());
  const [user, setUser] = useState<DemoUser | null>(() => readStoredUser());
  // true mientras el login automático desde redirect está en curso
  const [autoLoginPending, setAutoLoginPending] = useState(
    () => !!parseRedirectParams() && !readStoredUser(),
  );
  ...
  const [theme, setTheme] = useState<ThemeKey>(() => {
    return (
      (window.localStorage.getItem("olan-web:theme") as ThemeKey | null) ||
      "light"
    );
  });
  ...
  // Login silencioso desde redirect — sin pantalla de validación
  useEffect(() => {
    if (!redirectParams || user) {
      setAutoLoginPending(false);
      return;
    }
    try {
      if (redirectParams.proyecto) {
        window.localStorage.setItem(PROYECTO_KEY, redirectParams.proyecto);
      }

      const demoUser = buildRedirectUser(redirectParams);
      storeUser(demoUser);
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem("olan-web:theme", theme);
  }, [theme]);
  ...
  useEffect(() => {
    ...
    const CACHE_KEY = `alania-web:daily-suggestion:${user.id}`;
  ...
  const refreshConversations = useCallback(async () => {
    ...
    try {
      const [investigacion, practica] = await Promise.all([
        ...
        listConversationsBySourcePrefix(user.id, 'alania-practice-', user.token),
      ]);
      setConversations(investigacion);
      setPracicarConversations(practica);
    } catch (err) {
      if (err instanceof ApiError && err.status === 401 && user.refreshToken) {
        clearStoredUser();
        setUser(null);
        return;
      }
  ...
  function logout() {
    ...
    const target = proyecto
      ? `https://olan.com.co/${proyecto}/bienvenida.aspx`
      : 'https://olan.com.co';
    clearStoredUser();
    clearProfile();
    window.localStorage.removeItem(PROYECTO_KEY);
  ...
  useEffect(() => {
    ...
    window.addEventListener('olan:session-expired', handler);
    return () => window.removeEventListener('olan:session-expired', handler);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
    ...
    if (chatMode === "general") {
      ...
      try {
        ...
        const errorMessage = error instanceof Error && error.message.trim()
          ? error.message
          : "No pude conectar con olan-ai-gateway.";
        setMessages((prev) =>
          prev.map((msg) =>
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error && error.message.trim()
          ? error.message
          : "No pude conectar con olan-ai-gateway. Revisa el .env, el proxy de Vite o que el gateway este activo.";
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === assistantId
            ? {
  ...
  const investigacionConversations = useMemo(() =>
    syntheticConversations.filter(c =>
      !c.source || (!c.source.startsWith('alania-practice-') && c.source !== 'alania-voice-practice'),
    ),
  [syntheticConversations]);

  if (autoLoginPending || autoLoginError) {
    return (
      <main className="ol-shell" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '16px', minHeight: '100dvh' }}>
        {autoLoginError ? (
          <>
            <p style={{ color: 'var(--text-2)', fontSize: '0.9rem', textAlign: 'center', maxWidth: 340, margin: 0 }}>
              {autoLoginError}
            </p>
            <a
              href={getProyecto() ? `https://olan.com.co/${getProyecto()}` : 'https://olan.com.co'}
              style={{ color: '#00d4aa', fontSize: '0.85rem', textDecoration: 'none' }}
            >
              Volver a OLAN
            </a>
          </>
        ) : (
          <Loader2 size={32} style={{ animation: 'spin 1s linear infinite', color: '#00d4aa' }} />
        )}
      </main>
    );
  }
          ...
          <div className="hist-idiomas-scroll">
            ...
            <div className="conversation-list">
              {pracicarConversations.map((conv) => {
                const level = conv.source?.replace('alania-practice-', '') ?? 'Principiante';
                return (
                  <button
                    key={conv.id}
        ...
        <header className="topbar">
          ...
          <div className="topbar-left">
            ...
            <div className="topbar-brand">
              <img src={logoAiAlania} alt="" className="topbar-brand-img" />
              <div className="topbar-brand-text">
                <span className="topbar-brand-sub">AI LEARNING</span>
                <span className="topbar-brand-name">AlanIA</span>
              </div>
            </div>
              ...
              {loading && messages.length === 0 ? (
                ...
                <div className="welcome-panel">
                  ...
                  <div className="wp-hero">
                    <div className="wp-alania-wrap">
                      <div className="wp-alania-ring wp-alania-ring--outer" />
                      <div className="wp-alania-ring wp-alania-ring--inner" />
                      <div className="wp-alania-glow" />
                      <img src={alaniaHola} alt="AlanIA" className="wp-alania" />
                    </div>

                    <div className="wp-headline-block">
                      <p className="wp-greeting">
                        {user.name && !/^\d+$/.test(user.name.trim())
                          ? `Hola, ${user.name.split(' ')[0]} 👋`
                          : 'Hola 👋'}
                      </p>
                      <h1 className="wp-brand-title">
                        <span className="wp-hw wp-hw--1">AlanIA</span>
                      </h1>
                      <p className="wp-role-label">Tutor Virtual</p>
                    </div>

                    <div className="wp-caps-row">
                      {[
                        { icon: <Search size={13} />, label: 'Explorar temas', prompt: 'Quiero explorar el tema: ' },
                        { icon: <Lightbulb size={13} />, label: 'Organizar ideas', prompt: 'Ayúdame a organizar mis ideas sobre: ' },
                        { icon: <Brain size={13} />, label: 'Síntesis', prompt: 'Hazme una síntesis sobre: ' },
                      ].map(c => (
                        <button
                          key={c.label}
                          className="wp-cap-chip"
                          onClick={() => { setInput(c.prompt); wpSearchInputRef.current?.focus(); }}
                        >
                          {c.icon}
                          {c.label}
                        </button>
                    ...
                    return (
                      <div className={`wp-featured${suggestionLoading ? ' wp-featured--loading' : ''}`}>
                        <span className="wp-featured-badge">
                          <Sparkles size={11} /> SUGERENCIA DE ALANIA
            ...
            {!welcomeVisible && <form className="composer" onSubmit={sendMessage}>
              ...
              <div className="ctip-wrap">
                ...
                <div className="ctip ctip--left">
                  ...
                  <span>Pulsa y habla. AlanIA transcribirá tu pregunta automáticamente.</span>
                </div>
              </div>
              ...
              <div className="ctip-wrap">
                ...
                <div className="ctip ctip--left">
                  ...
                  <span>Habla con AlanIA directamente, sin necesidad de escribir nada.</span>
                </div>
              </div>
              ...
              <div className="ctip-wrap">
                ...
                <div className="ctip ctip--right">
                  ...
                  <span>Envía tu pregunta a AlanIA. También puedes usar Enter ↵.</span>
                </div>
              </div>
            </form>}
          </>

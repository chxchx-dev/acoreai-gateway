...
function resolveDevGatewayUrl() {
  return (
    cleanUrl(import.meta.env.VITE_OLAN_AI_GATEWAY_LOCAL_URL) ||
    cleanUrl(import.meta.env.VITE_OLAN_AI_GATEWAY_URL) ||
    'http://ai.217-77-0-2.nip.io:4005'
  );
}

function resolveGatewayUrl() {
  const gatewayMode = String(import.meta.env.VITE_OLAN_AI_GATEWAY_MODE || 'publico').toLowerCase();
  const usePublicGateway = ['public', 'publico', 'production', 'produccion'].includes(gatewayMode);
  const configuredUrl = (
    cleanUrl(
      usePublicGateway
        ? import.meta.env.VITE_OLAN_AI_GATEWAY_PUBLIC_URL
        : import.meta.env.VITE_OLAN_AI_GATEWAY_LOCAL_URL,
    ) ||
    cleanUrl(import.meta.env.VITE_OLAN_AI_GATEWAY_URL) ||
    resolveDevGatewayUrl()
  );

  // Si la web corre en HTTPS y el gateway configurado es HTTP, el navegador
  // bloqueará la llamada por Mixed Content. En producción web/olan ya expone
  // el gateway por same-origin bajo /api, así que aquí debemos usar base vacía.
  if (
...
function resolveGatewayKey() {
  ...
  return String(
    (usePublicGateway
      ? import.meta.env.VITE_OLAN_AI_GATEWAY_PUBLIC_KEY
      : import.meta.env.VITE_OLAN_AI_GATEWAY_LOCAL_KEY) ||
      '',
  ).trim();
}

const apiBase = resolveGatewayUrl();
const authApiBase = resolveGatewayUrl();
const gatewayApiKey = resolveGatewayKey();

const gatewayJsonHeaders: Record<string, string> = {
  'Content-Type': 'application/json',
  ...(gatewayApiKey ? { 'x-ai-gateway-key': gatewayApiKey } : {}),
};

const OLAN_PLATFORM_LOGIN_URL =
  'https://olan.com.co/app/api.asmx/GetAccess';
const OLAN_PLATFORM_USER_BY_ID_URL =
  'https://olan.com.co/app/api.asmx/GetAccessOnlyID';
...
type OlanPlatformLoginRow = {
  ...
  idestudiante?: string;
  confgeneral?: Array<Record<string, unknown>> | Record<string, unknown>;
  [key: string]: unknown;
};

function firstOlanResult(raw: string): OlanPlatformLoginRow {
  let data: { result?: OlanPlatformLoginRow[] } = {};
  try {
    data = raw ? (JSON.parse(raw) as { result?: OlanPlatformLoginRow[] }) : {};
  } catch {
    throw new Error('La plataforma OLAN devolvio una respuesta invalida.');
  }
...
function hasModiaEnabled(row: OlanPlatformLoginRow): boolean {
  const confgeneral = Array.isArray(row.confgeneral)
    ? row.confgeneral[0]
    : row.confgeneral;
  const modia = confgeneral && typeof confgeneral === 'object'
    ? (confgeneral as Record<string, unknown>).modia
    : undefined;
  return String(modia ?? '').trim().toLowerCase() === 'si';
}

function assertModiaEnabled(_row: OlanPlatformLoginRow) {
  // web/olan permite acceso sin bloquear por la bandera modia del proyecto.
  return;
}

async function verifyOlanPlatformLogin(
  identificacion: string,
  password: string,
): Promise<OlanPlatformLoginRow> {
  const url =
    `${OLAN_PLATFORM_LOGIN_URL}?uEstudiante=${encodeURIComponent(identificacion)}` +
    `&pEstudiante=${encodeURIComponent(password)}`;
  const res = await fetch(url);
  const raw = await res.text().catch(() => '');

  if (!res.ok) {
    throw new ApiError(`OLAN respondio ${res.status}`, res.status);
  }

  const first = firstOlanResult(raw);
  assertModiaEnabled(first);
  return first;
}

async function verifyOlanAiAccessById(identificacion: string): Promise<OlanPlatformLoginRow> {
  const url = `${OLAN_PLATFORM_USER_BY_ID_URL}?uEstudiante=${encodeURIComponent(identificacion)}`;
  const res = await fetch(url);
  const raw = await res.text().catch(() => '');
...
const DEVICE_ID_KEY = 'olan-web:device-id';
  ...
  const res = await authenticatedFetch(`${apiBase}/api/chat/stream`, {
    ...
    body: JSON.stringify({
      ...
      source: payload.mode === 'practice'
        ? `olan-practice-${payload.practiceLevel ?? 'Principiante'}`
        : payload.mode === 'voice'
          ? 'olan-voice'
          : 'olan-web',
      useRag: false,
  ...
  while (true) {
    ...
    for (const raw of events) {
      ...
      if (parsed.event === 'error') {
        throw new Error(String(parsed.data.message ?? 'Error en el stream de AlanIA'));
      }
    }
  }
...
export async function loginUser(
  identificacion: string,
  password: string,
): Promise<{ token: string; refreshToken?: string; proyecto?: string; user: { id: string; email: string; name: string; role: string } }> {
  const row = await verifyOlanPlatformLogin(identificacion, password);
  const realId = String(row.identificacion ?? row.idestudiante ?? identificacion).trim() || identificacion;
  const name    = String(row.nombre ?? row.name ?? realId).trim() || realId;
  const proyecto = row.proyecto ? String(row.proyecto).trim() : undefined;
  return {
    token: '',
    proyecto,
    user: {
      id:    realId,
      email: `${realId}@olan.internal`,
      name,
      role:  'ACADEMIC',
    },
  };
...
export async function loginOlanToken(
  urlParam: string,
  olanToken: string,
  opts?: { force?: boolean },
): Promise<{ token: string; refreshToken: string; proyecto?: string; identificacion?: string; user: { id: string; email: string; name: string; role: string } }> {
  // The AI gateway backend validates the OLAN token server-side — no browser→OLAN call needed.
  // Passing urlParam as identificacion; the backend resolves the canonical cedula internally.
  const res = await fetch(`${authApiBase}/api/auth/login-olan-token`, {
    method: 'POST',
    headers: gatewayJsonHeaders,
    body: JSON.stringify({
      identificacion: urlParam,
      olanToken,
...
function dispatchSessionExpired() {
  clearStoredUser();
  window.dispatchEvent(new CustomEvent('olan:session-expired'));
}
...
async function refreshStoredSession(): Promise<string | null> {
  ...
  refreshPromise = (async () => {
    ...
    const res = await fetch(`${authApiBase}/api/auth/refresh`, {
      method: 'POST',
      headers: gatewayJsonHeaders,
...
async function authenticatedFetch(
  ...
  if (gatewayApiKey) headers.set('x-ai-gateway-key', gatewayApiKey);
  ...
  if (gatewayApiKey) retryHeaders.set('x-ai-gateway-key', gatewayApiKey);
...
export async function transcribeAudio(
  ...
  const res = await fetch(`${apiBase}/api/stt?language=${encodeURIComponent(language)}`, {
    method: 'POST',
    headers: gatewayApiKey ? { 'x-ai-gateway-key': gatewayApiKey } : undefined,
    body: form,
    signal,
  });
  if (!res.ok) throw await parseError(res);
...
export async function generateDailySuggestion(
  ...
  const res = await authenticatedFetch(`${apiBase}/api/chat/stream`, {
    ...
    body: JSON.stringify({
      ...
      useHistory: false,
      source: 'olan-system',
      system: 'Eres el motor de sugerencias diarias de AlanIA. Respondes ÚNICAMENTE con un objeto JSON válido. Sin explicaciones, sin markdown, sin texto extra.',
      keepAlive: '2m',

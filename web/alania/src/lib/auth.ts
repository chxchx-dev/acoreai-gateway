...
export function readStoredUser(): DemoUser | null {
  const raw = window.localStorage.getItem('alania-web:user');
  if (!raw) return null;

  try {
    return JSON.parse(raw) as DemoUser;
  } catch {
    window.localStorage.removeItem('alania-web:user');
    return null;
  }
}

export function storeUser(user: DemoUser) {
  window.localStorage.setItem('alania-web:user', JSON.stringify(user));
}

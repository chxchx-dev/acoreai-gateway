  ...
  const res = await authenticatedFetch(`${apiBase}/api/chat/stream`, {
    ...
    body: JSON.stringify({
      ...
      source: payload.mode === 'practice'
        ? `alania-practice-${payload.practiceLevel ?? 'Principiante'}`
        : payload.mode === 'voice'
          ? 'alania-voice'
          : 'alania-web',
      useRag: false,
...
const DEVICE_ID_KEY = 'alania-web:device-id';
...
export async function loginUser(
  ...
  password: string,
  opts?: { force?: boolean },
): Promise<{ token: string; refreshToken: string; user: { id: string; email: string; name: string; role: string } }> {

...
// The original app keeps these optional illustrations disabled until their
// assets are supplied. Empty sources preserve that behavior while keeping the
// unified build type-safe.
const acoreaiHola = '';
const acoreaiIdea = '';
const acoreaiNopasa = '';
const acoreaiUps = '';
...
const MOOD_ART: Record<Mood, string> = {
  hola:     acoreaiHola,
  pensando: acoreaiIdea,
  timida:   acoreaiNopasa,
  ups:      acoreaiUps,
};
...
export function LoginScreen({ onLogin }: LoginScreenProps) {
  const REMEMBER_KEY = 'acoreai-web:remember-email';
      ...
      <section className="lg-scene">
        <div className="lg-hero" aria-hidden="true">
          ...
          {MOODS.map((key) => (
            <img
              key={key}
              className={`lg-acoreai${mood === key ? ' is-active' : ''}`}
              src={MOOD_ART[key]}
              alt=""
            />
          ))}

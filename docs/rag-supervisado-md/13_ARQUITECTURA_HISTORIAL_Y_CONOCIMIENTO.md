...
`olan-ai-gateway` ya corría con **dos bases de datos físicamente separadas** desde antes
...
(`DATABASE_URL`), definidas en `prisma/schema.prisma`. Sin `tenant_id`: Alania y
Olan corren en gateways separados, así que no aplica multi-tenant.
...
| `alania-web`, `olan-web`, `olan-system`, etc. | `ChatService` (chat general) | conversación del producto Alania/Olan, no relacionada al RAG |
